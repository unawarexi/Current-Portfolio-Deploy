...
const Section = ({ title, children }) => (
  <motion.div variants={staggerItemBig} className="py-12 md:py-16 border-b border-gray-200 dark:border-white/[0.06] last:border-0">
    <h3 className="text-sm md:text-base font-bold uppercase tracking-[0.2em] text-primary-500 mb-6 md:mb-8">{title}</h3>
    {children}
  </motion.div>
);
...
const ListBullet = ({ items }) => (
  <ul className="space-y-4 md:space-y-6">
    {items.map((item, i) => (
      <li key={i} className="flex items-start gap-4 md:gap-6 text-base md:text-lg lg:text-xl text-gray-700 dark:text-gray-300 leading-[1.8]">
        <div className="flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-full bg-primary-500/10 flex items-center justify-center mt-1">
...
const SingleExperience = () => {
  ...
  return (
    <div className="bg-white dark:bg-[#070b18] min-h-screen relative overflow-hidden">
      ...
      {/* Hero band */}
      <div className="relative bg-gray-950 overflow-hidden pt-24 pb-16 md:pt-32 md:pb-24">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 sm:px-10 lg:px-20">
          ...
          <motion.h1
            ...
            className="font-display text-4xl sm:text-5xl md:text-7xl font-extrabold text-white tracking-tight leading-[1.1] mb-8"
          >
            {item.role}
          </motion.h1>

          <div className="flex flex-wrap items-center gap-6 md:gap-10 text-white/60 text-base md:text-xl font-medium">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="relative z-10 max-w-5xl mx-auto px-6 sm:px-10 lg:px-20 py-16 md:py-24"
      >
        <Section title="Overview">
          <p className="text-gray-600 dark:text-gray-300 leading-[1.8] text-lg md:text-xl lg:text-2xl font-light">
            {item.description}
          </p>
        </Section>
        ...
        {item.impact && (
          <Section title="Impact & Contribution">
            <div className="p-8 md:p-12 rounded-[2rem] bg-primary-50 dark:bg-primary-500/5 border border-primary-100 dark:border-primary-500/10">
          ...
          <Section title="Products Built">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              {productsBuilt.map((p, i) => (
                <div key={i} className="flex items-center gap-4 p-6 md:p-8 rounded-2xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.06] hover:-translate-y-1 transition-transform">
                  <span className="flex-shrink-0 w-10 h-10 rounded-xl bg-burgundy-500/10 text-burgundy-600 dark:text-burgundy-400 font-display text-lg font-bold flex items-center justify-center">0{i + 1}</span>
                  <span className="text-base md:text-lg font-medium text-gray-800 dark:text-gray-200">{p}</span>
                </div>
              ))}
            </div>
          </Section>
        ...
        {skills.length > 0 && (
          <Section title="Technologies Used">
            <div className="flex flex-wrap gap-3 md:gap-4">
              {skills.map((s) => (
                <span key={s} className="px-5 py-2.5 rounded-full text-sm md:text-base font-semibold bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                  {s}
                </span>
              ))}
            </div>
        ...
        {/* CTA */}
        <div className="pt-24 md:pt-32 pb-12 flex items-center justify-between flex-wrap gap-6">
          <Link
            to="/#experience"
