...
const TABS = [
  ...
  { id: "content", label: "Content", fields: ["description", "impact"] },
  {
    id: "lists",
    label: "Lists",
    fields: ["skills", "technologies", "achievements", "productsBuilt"],
  },
];
...
const inputCls =
  "w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:outline-none transition";
    ...
    <div className="max-w-2xl mx-auto bg-white dark:bg-gray-950 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-xl overflow-hidden">
          ...
          >
            ...
            {activeTab === "basics" && (
              <>
                <Field
                  ...
                  error={uc.formErrors.company}
                >
                  <input
                    className={inputCls}
                    value={uc.form.company}
                    onChange={(e) => uc.setField("company", e.target.value)}
                    placeholder="e.g. Dowell Technologies"
                  />
                </Field>
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Role / Title *" error={uc.formErrors.role}>
                    <input
                      className={inputCls}
                      value={uc.form.role}
                      onChange={(e) => uc.setField("role", e.target.value)}
                  ...
                  <Field label="Year / Period *" error={uc.formErrors.year}>
                    <input
                      className={inputCls}
                      value={uc.form.year}
                      onChange={(e) => uc.setField("year", e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Type" error={uc.formErrors.type}>
                    <select
                      className={inputCls}
                      value={uc.form.type}
                      onChange={(e) => uc.setField("type", e.target.value)}
                    >
                      {TYPES.map((t) => (
                  ...
                  <Field
                    label="Sub-category"
                    error={uc.formErrors.subCategory}
                    hint="e.g. Frontend, Fullstack, Backend"
                  >
                    <input
                      className={inputCls}
                      value={uc.form.subCategory}
                      onChange={(e) =>
                        uc.setField("subCategory", e.target.value)
                      }
                ...
                <Field label="Sort Order" hint="Lower number = appears first">
                  <input
                    ...
                    className={inputCls}
                    value={uc.form.order}
                    onChange={(e) => uc.setField("order", e.target.value)}
                  />
                </Field>
              </>
            )}
            ...
            {activeTab === "content" && (
              <>
                <Field
                  label="Description *"
                  error={uc.formErrors.description}
                  hint="Describe what you did at this company"
                >
                  <textarea
                    rows={5}
                    className={textareaCls}
                    value={uc.form.description}
                    onChange={(e) => uc.setField("description", e.target.value)}
                    placeholder="I was responsible for..."
                  />
                </Field>
                <Field
                  label="Impact / Key Contribution"
                  error={uc.formErrors.impact}
                  hint="Quantify your impact where possible"
                >
                  <textarea
                    rows={4}
                    className={textareaCls}
                    value={uc.form.impact}
                    onChange={(e) => uc.setField("impact", e.target.value)}
                    placeholder="Increased performance by 40%, led a team of 5..."
                  />
                </Field>
              </>
            )}
            ...
            {activeTab === "lists" && (
              <>
                <DynamicFieldList title="Skills Used" items={uc.form.skills} onChange={(v) => uc.setField('skills', v)} placeholder="E.g. React" />
                <DynamicFieldList title="Technologies" items={uc.form.technologies} onChange={(v) => uc.setField('technologies', v)} placeholder="E.g. Node.js" />
                <DynamicFieldList title="Achievements" items={uc.form.achievements} onChange={(v) => uc.setField('achievements', v)} placeholder="E.g. Delivered project 2 weeks early" />
                <DynamicFieldList title="Products Built" items={uc.form.productsBuilt} onChange={(v) => uc.setField('productsBuilt', v)} placeholder="E.g. Customer dashboard" />
              </>
            )}
          </motion.div>
        </AnimatePresence>
      ...
      <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button
            onClick={uc.reset}
            className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-gray-500 hover:text-gray-700 transition"
          >
