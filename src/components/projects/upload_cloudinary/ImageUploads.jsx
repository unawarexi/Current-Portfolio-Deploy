...
const ImageUploads = () => {
  ...
  const handleRemoveFile = (fileType, index) => {
    if (fileType === "cover") {
      ...
      // console.log(
      //   `Removed cover photo at index ${index}, remaining: ${updatedFiles.length}`
      // );
      ...
      // console.log(
      //   `Removed project image at index ${index}, remaining: ${updatedFiles.length}`
      // );
    }
  };
  ...
  const handleUploadAllImages = async () => {
    ...
    try {
      ...
      if (formData.profilePicture) {
        setUploadProgress((prev) => ({ ...prev, profile: 10 }));
        const profileResponse = await uploadImageToCloudinary(
          formData.profilePicture
        );
        setUploadProgress((prev) => ({ ...prev, profile: 100 }));
      ...
      if (formData.coverPhotoUpload.length > 0) {
        setUploadProgress((prev) => ({ ...prev, cover: 10 }));
        const coverResponse = await uploadMultipleImages(
          formData.coverPhotoUpload
        );
        setUploadProgress((prev) => ({ ...prev, cover: 100 }));
      ...
      if (formData.projectImageUpload.length > 0) {
        setUploadProgress((prev) => ({ ...prev, project: 10 }));
        const projectResponse = await uploadMultipleImages(
          formData.projectImageUpload
        );
        setUploadProgress((prev) => ({ ...prev, project: 100 }));
    ...
    <>
      ...
      <div className="shadow-lg pb-6 mx-auto bg-white dark:bg-[#1a1a2e] my-6 px-12 py-8 rounded-xl border-2 border-gray-300 dark:border-gray-700">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-300 mb-6">
          This information will help potential employers learn more about you
          and your skills.
        </p>
          ...
          <button
            ...
            className={`w-full p-2 rounded-md text-white ${
              loading
                ...
                : "bg-indigo-600 hover:bg-indigo-500 dark:bg-indigo-800 dark:hover:bg-indigo-700"
            }`}
            disabled={loading}
          >
            {loading ? (
